...
/** Generate a new id and return a copy.
 *
 * @return guid A data structure containing a newly created GncGUID.
 */
GncGUID guid_new_return (void);

/** Returns a GncGUID which is guaranteed
 *  to never reference any entity. Do not free this value. The same pointer
 *  is returned on each call.
 */
const GncGUID * guid_null (void);

/** Allocate and initialize a new guid, and return
 *  a pointer to it. Caller must call guid_free afterward to
...
/** Allocates a new guid as a copy of the one passed in. The caller
 *  must free this pointer using guid_free.
 */
 ...
 *  returned memory is owned by the caller and must be freed by him.
 */
...
/** Given a string, replace the given guid with the parsed one unless the
 * given value is null.
 * If null is passed as guid or string, false is returned and nothing
 * is done, otherwise, the function returns true.
 * This function accepts both upper and lower case
 * hex digits. If letters outside the range of [a-fA-F] are passed, they
 * silently replaced. If non-alphanumeric digits are given, this function
 * might throw a c++ exception.
 */
